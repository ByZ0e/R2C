...
def get_grid(pose, grid_size, device):
    ...
    #                        torch.zeros(cos_t.shape).float().to(device)], 1)
    theta11 = torch.stack([cos_t, -sin_t,
                           torch.zeros(cos_t.shape).float().cuda()], 1)
    theta12 = torch.stack([sin_t, cos_t,
                           torch.zeros(cos_t.shape).float().cuda()], 1)
    ...
    theta21 = torch.stack([torch.ones(x.shape).cuda(),
                           -torch.zeros(x.shape).cuda(), x], 1)
    theta22 = torch.stack([torch.zeros(x.shape).cuda(),
                           torch.ones(x.shape).cuda(), y], 1)
