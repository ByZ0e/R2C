...
from alfred.gen.utils import augment_util_ziyi, video_util
...
def setup_task(env, traj_data, args):
    ...
    augment_util_ziyi.check_image(env.last_event.frame)
...
def augment_traj(env, json_file, args, video_saver, render_settings):
    ...
    action_images_orig = [None] * len(traj_data['plan']['low_actions'])  # 初始化一份空的image list, 长度保持和action steps一致
    for image_dict in traj_data['images']:
        # 存入原始image信息: {"high_idx": 0, "image_name": "000000000.png", "low_idx": 0}
        if action_images_orig[image_dict['low_idx']] is None:
            action_images_orig[image_dict['low_idx']] = image_dict  
    traj_data['images'] = list()
    traj_data['agent_poses'] = list()

    root_dir_to, rendered_images_dir, save_settings = augment_util_ziyi.prepare_for_traj(
        json_file, args)
    ...
    for ll_idx, ll_action in enumerate(traj_data['plan']['low_actions']):
        ...
        if ('MoveAhead' in cmd['action'] or
            ...
            'Look' in cmd['action']):
            event, img_count = augment_util_ziyi.env_navigate(
        ...
        # 'CoolObject' action is actually 'CloseObject'
            ...
            event, img_count = augment_util_ziyi.env_interact(
                cmd, env, save_settings, root_dir_to,
        ...
        # update agent pose list 
        for j in range(img_count - img_count_before):
            traj_data['agent_poses'].append(event.metadata['agent'])
    ...
    augment_util_ziyi.save_image(env.last_event, root_dir_to, save_settings, img_count)
    img_count += 10
    ...
    # write an updated traj_data.json (updated images, colors, rewards and agent poses)
    ...
    augment_util_ziyi.write_compressed_images(args, root_dir_to)
    return True
...
def main(args):
    ...
    processed_files_path = os.path.join(args.data_output, 'reprocessed.txt')
    ...
    traj_list.append('train/pick_and_place_simple-Potato-None-Microwave-30/trial_T20190907_020007_658755/traj_data.json')
